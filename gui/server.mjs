...
import { ttsSpeak, proxied, TTS_DIR, streamClips, limiterStats } from './lib/tts.mjs';
...
async function handle(req, res) {
  ...
  if (req.method === 'GET' && (url === '/' || url === '/index.html')) {
    try {
      const html = (await readFile(join(__dir, 'index.html'), 'utf8')).replace('</head>', `<script>window.CC_STT_LIVE=${process.env.CC_STT_LIVE === '1'};</script></head>`);
  ...
  if (req.method === 'POST' && url === '/followup') {
    ...
    const invite = validated.length ? bridging.inviteText(validated[0], userKey) : 'Möchten Sie noch etwas aus dem Deutschlandatlas wissen?';
    let inviteAudioUrl = null;
    try { inviteAudioUrl = proxied(await ttsSpeak(invite, { priority: false, userKey })); } catch {}
