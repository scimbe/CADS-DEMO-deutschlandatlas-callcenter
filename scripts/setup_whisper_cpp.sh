...
# model (never an English-only ".en" model — this repo transcribes German).
#
# Same pattern as CADS-DEMO-podcast/scripts/setup_whisper_cpp.sh. On failure
# of the model download (no network, blocked host, 4xx/5xx), this script
# exits non-zero and prints exactly what's missing and why — it never
# silently substitutes a mock.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VENDOR_DIR="$REPO_ROOT/vendor/whisper.cpp"
PINNED_TAG="b4938"
# Multilingual model. "base" (~148MB) is the demo default: fast to fetch/run
# and comfortably better than "tiny" for German. For production-grade German
# accuracy, re-run with WHISPER_MODEL_NAME=small (~488MB) or "medium" (~1.5GB).
MODEL_NAME="${WHISPER_MODEL_NAME:-base}"

case "$MODEL_NAME" in
  *.en) echo "FATAL: MODEL_NAME=$MODEL_NAME is English-only; this repo needs a multilingual model (no .en suffix)." >&2; exit 1 ;;
esac
...
if [ -f "$MODEL_PATH" ]; then
  ...
  if ! ( cd "$VENDOR_DIR" && bash "models/download-ggml-model.sh" "$MODEL_NAME" ); then
    ...
    echo "refuse to run without this model (see src/callcenter_speech/stt.py)." >&2
    exit 1
  fi
fi
